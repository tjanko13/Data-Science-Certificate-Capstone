...
## Naive Bayes
```{r}
# too slow, figure something out!
# dat_train_mod = ceiling(dat_dtm_reduced_matrix)
# dat_predict_mod = ceiling(dat_dtm_reduced_matrix_predict)
...
#                 trControl = train_control)
# 
# predictions_nb = predict(mod_nb$finalModel, dat_dtm_reduced_matrix_predict,
#                          type = 'class')
...
predictions_knn = predict(mod_knn$finalModel, dat_dtm_reduced_matrix_predict, 
                          type = 'class')
confusionMatrix(predictions_knn, as.factor(can_predict$reviewer_recommended))
```
...
## looking into random forest (if chosen as model, this should be seen)
```{r}
mod_rf$finalModel %>%

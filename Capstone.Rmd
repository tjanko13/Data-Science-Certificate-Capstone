...
dat_tfidf = dat %>%
  count(reviewer_recommended, word) %>%
  bind_tf_idf(term = word, document = reviewer_recommended, n = n)
dat_tfidf %>% head()
```
...
plot_dt %>%
  ...
  top_n(10) %>%
  ungroup() %>%
  mutate(word = reorder_within(word, tf_idf, reviewer_recommended)) %>%
  ggplot(aes(word, tf_idf)) +
  geom_col() + 
  scale_x_reordered() + 
  labs(x = NULL, y = "tf-idf") + 
  facet_wrap(~reviewer_recommended, scales = "free") +
  coord_flip()
```
...
# smp_size = floor(training_split * nrow(dat))
# dat_index = sample(seq_len(nrow(dat)), size = smp_size)
# dat_train = as.data.frame(dat[dat_index,])
# dat_test = as.data.frame(dat[-dat_index,])


mod = train(x = as.matrix(dat_dtm_reduced),
            y = raw_dat$reviewer_recommended,
            method = "ranger",
            num.trees = 10,
            importance = "impurity",
...
#                      classProbs = TRUE)
# 
# mod_dt = train(dat_train %>% select(reviewer_recommended), 

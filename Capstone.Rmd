...
dat = dat_train %>%
  ...
  rename(afinn = value) %>%
  mutate(word = SnowballC::wordStem(word)) #brings it down to the root word

# this should be done by bigram :)
dat_sent = dat %>%
  group_by(id) %>%
  summarize(sentiment = mean(afinn, na.rm = TRUE))

dat_sent$sentiment[is.na(dat_sent$sentiment)] = 0

dat_train_sent = dat_train %>%
  left_join(dat_sent, by = 'id')
...
dat_dtm_reduced = tm::removeSparseTerms(dat_dtm, sparse = 0.90)

# If you needed to add a feature you joined...
# haha = dat_train %>% 
#   mutate(blah = if_else(reviewer_recommended == 'yes', 1, 0)) %>%
#   select(blah) %>%
#   pull()
# mod = train(x = cbind(as.matrix(dat_dtm_reduced), haha),

dat_dtm_reduced_matrix = cbind(as.matrix(dat_dtm_reduced), data_sentiment = dat_train_sent$sentiment)
...
dat_dtm_reduced_predict = tm::removeSparseTerms(dat_dtm_predict, sparse = 0.95)
dat_dtm_reduced_matrix_predict = as.matrix(dat_dtm_reduced_predict)

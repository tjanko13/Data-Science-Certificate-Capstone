...
#dat_model <- dat_model %>% 
#  gsub("\\s*\\([^\\)]+\\)","",as.character(dat_model$beer_name))
```
...
model_rf <- train(dat_train_up_rf %>% select(-Class, cluster),
                  dat_train_up_rf$Class,
                  method = 'ranger',
                  trControl = train_control, respect.unordered.factors=TRUE)
...
```{r}
predictions <- predict(model_pp, dat_test_rf)
predictions
```

```{r}
model_rf_pp <- preProcess(dat_train_up_rf, method = c('center', 'scale'))
dat_train_rf_pp <- predict(model_rf_pp, dat_train_up_rf)
dat_train_rf_pp
```

```{r}
dat_test_rf_pp <- predict(model_rf_pp, dat_test_rf)
dat_test_rf_pp
```

```{r}
predictions <- predict(model_rf, dat_test_rf_pp)
predictions
```

```{r}
pred_test <- predict(model_pp, dat_test_rf)
...
```{r}
confusionMatrix(predictions1, dat_test_rf)
```

```{r}
confusionMatrix(predictions, dat_test_rf_pp$Class)
```

```{r}
df_new_beer <- as.data.frame()
```

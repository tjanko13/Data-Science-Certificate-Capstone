...
## Sampling the train set for easy viewing   
```{r}
#dat_train_rownames <- dat_train_rownames %>% 
#  sample_n(50)
```
...
```{r}
pp_model <- preProcess(dat_train_rownames, method = c('center', 'scale'))
dat_train_pp <- predict(pp_model, dat_train_rownames)
dat_train_pp
```
...
```{r}
distance_data <- dist(dat_train_pp, method = 'euclidean')
...
```{r}
pred_test <- predict(pp_model, dat_test_rownames)
...
```{r}
predictions <- predict(pp_model, dat_test_standardized)
predictions
```
